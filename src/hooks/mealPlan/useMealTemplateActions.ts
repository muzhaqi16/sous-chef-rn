...
} from '../../graphql/operations/mealPlan/mealTemplate.generated';
import {
...
const addToMealTemplates = createAddToQueryConnectionUpdater<{ id: string }>(
  'mealTemplates',
  'MealTemplate',
);
...
export function useMealTemplateActions() {
  const [createFromTemplateMutation, { loading: creatingFromTemplate }] =
    useMutation(CreateMealPlanFromTemplateDocument, {
      update(cache, { data }) {
        const newPlan = data?.createMealPlanFromTemplate?.mealPlan;
  ...
  const [createTemplateMutation, { loading: creatingTemplate }] = useMutation(
    ...
    {
      update(cache, { data }) {
        const newTemplate = data?.createTemplateFromMealPlan?.mealTemplate;
        if (newTemplate) addToMealTemplates(cache, newTemplate);
      },
      onError: error => {
  ...
  const [deleteTemplateMutation, { loading: deleting }] = useMutation(
    DeleteMealTemplateDocument,
    {
      update(cache, { data }, { variables }) {
        const id = data?.deleteMealTemplate?.mealTemplate?.id ?? variables?.id;
        if (id) removeFromMealTemplates(cache, id, { evictItem: true });
      },
      onError: error => {
  ...
  const [duplicateTemplateMutation, { loading: duplicating }] = useMutation(
    ...
    {
      update(cache, { data }) {
        const newTemplate = data?.duplicateTemplate?.mealTemplate;
        if (newTemplate) addToMealTemplates(cache, newTemplate);
      },
      onError: error => {
