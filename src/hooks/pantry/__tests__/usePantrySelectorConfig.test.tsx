...
describe('usePantrySelectorConfig', () => {
  const defaultOptions = {
    pantries: [{ id: 'p1', name: 'Kitchen' }, { id: 'p2', name: 'Garage' }],
    selectedPantryId: 'p1',
    loading: false,
    setSelectedPantryId: jest.fn(),
    selectorRef: { current: { open: jest.fn(), close: jest.fn(), isActive: jest.fn(), toggle: jest.fn() } },
    navigate: jest.fn(),
  };

  it('returns config with correct title', () => {
    const { result } = renderHook(() => usePantrySelectorConfig(defaultOptions));
  ...
  it('disables edit/analytics when no pantry selected', () => {
    const { result } = renderHook(() =>
      usePantrySelectorConfig({ ...defaultOptions, selectedPantryId: undefined }),
    );
