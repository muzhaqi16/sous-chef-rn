...
export function useStorageLocationManagement(homeId: string | undefined) {
  ...
  // Mutations
  const [createMutation, { loading: creating }] = useCreateStorageLocationMutation({
    update: (cache, { data }) => {
        ...
        if (existingData?.storageLocations) {
          ...
          if (!exists) {
            ...
            cache.writeQuery({
              ...
              variables: { homeId },
              data: {
                storageLocations: [...existingData.storageLocations, newLocation],
              },
            });
          }
        }
  ...
  const [updateMutation, { loading: updating }] = useUpdateStorageLocationMutation({
  ...
  return {
    ...
    tree: treeData?.storageLocationTree ?? [],
    loading,
