...
describe('shoppingListPermissions', () => {
  // The id comparison behind ownership is private now; exercise it where it
  // actually decides something, which is the only place a caller can reach it.
  describe('ownership short-circuits every other permission', () => {
    it('grants full permissions to the owner', () => {
      const permissions = getShoppingListPermissionsWithOwner(
        { ownership: { userId: 'u1' } },
        'u1',
      );
      expect(permissions.canAddItems).toBe(true);
      expect(permissions.canRemoveItems).toBe(true);
      expect(permissions.canEditItems).toBe(true);
      expect(permissions.canMarkPurchased).toBe(true);
    });

    it('does not grant them to a different user', () => {
      const permissions = getShoppingListPermissionsWithOwner(
        { ownership: { userId: 'u1' } },
        'u2',
      );
      expect(permissions.canRemoveItems).toBe(false);
    });

    it('does not grant them when the viewer is anonymous', () => {
      const permissions = getShoppingListPermissionsWithOwner(
        { ownership: { userId: 'u1' } },
        undefined,
      );
