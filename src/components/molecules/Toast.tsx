...
const animateDismiss = (
  ...
  translateY.set(
    withSpring(TOAST.OFFSCREEN_Y, motion.spring.TOAST_DISMISS, finished => {
      'worklet';
      if (finished) {
        translateX.set(0);
  ...
  opacity.set(withTiming(0, { duration: motion.timing.STANDARD }));
};
    ...
    >
      ...
      {actionLabel ? (
        <Pressable onPress={onActionPress} style={styles.actionButton}>
          <Text role="bodyStrong" style={styles.actionText}>
            {actionLabel}
          </Text>
        </Pressable>
      ) : null}
...
/**
 * Holds the toast queue and registers the imperative dispatch. No context, so
 * `children` is optional: mounted as a LEAF last at the app root, the card
 * lands last in the native order. Wrapping the navigator put it under every
 * header — and under the iOS 26 glass, which composites above RN siblings.
 */
export const ToastProvider: React.FC<{ children?: ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    translateY.set(withSpring(0, motion.spring.TOAST_ENTER));
    translateX.set(0);
    opacity.set(withTiming(1, { duration: motion.timing.FAST }));
  ...
  const panGesture = usePanGesture({
    ...
    onDeactivate: event => {
      ...
      if (shouldDismiss) {
        ...
        translateY.set(withSpring(0, motion.spring.TOAST_ENTER));
        translateX.set(withSpring(0, motion.spring.TOAST_ENTER));
      }
    },
  });
