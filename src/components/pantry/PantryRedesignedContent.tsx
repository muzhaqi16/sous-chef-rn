...
import { PantryLocationTabs, LocationFilter } from './PantryLocationTabs';
...
interface PantryRedesignedContentProps {
  // User info
  userName: string;
...
export const PantryRedesignedContent: React.FC<PantryRedesignedContentProps> = ({
  userName,
  householdName,
  avatarInitial,
  avatarUrl,
  ...
  const getLocationString = useCallback((storageState?: string | null): string => {
    switch (storageState) {
  ...
  const renderItemCard = useCallback(
    (item: PantryItem, variant: ItemVariant = 'normal') => {
      ...
      const emoji = getCategoryEmoji(item.item?.category?.name);
      const unitSymbol = item.unit?.symbol || item.item?.displayUnit?.symbol || 'pcs';
      const quantity = formatQuantityDisplay(item.currentQuantity, unitSymbol);
      ...
      return (
        <PantryItemCard
          ...
          expirationVariant={expStatus.type}
          quantity={quantity}
          location={location}
          variant={variant}
          imageUrl={imageUrl}
    ...
    ({ item }: ListRenderItemInfo<ListSection>) => {
      switch (item.type) {
        ...
        case 'item':
          ...
          return renderItemCard(item.data, isExpiringSoon ? 'warning' : 'normal');
        default:
          return null;
      }
    },
  ...
  const ListHeader = useMemo(
    () => (
      <View>
        ...
        {/* Expired banner */}
        {expiredCount > 0 && (
          <ExpiredBanner count={expiredCount} onPress={onExpiredBannerPress} />
        )}
      </View>
    ),
    [locationFilter, onLocationFilterChange, locationCounts, expiredCount, onExpiredBannerPress],
  );
            ...
            <TouchableWithoutFeedback>
              <View style={styles.sortModal}>
                ...
                {[
                  ...
                  { key: 'expiry' as SortOption, label: 'Expiry Date', icon: '📅' },
                  { key: 'quantity' as SortOption, label: 'Quantity', icon: '📊' },
                  { key: 'recent' as SortOption, label: 'Recently Added', icon: '🕐' },
                ].map(option => (
                  <TouchableOpacity
                    key={option.key}
                    style={[
