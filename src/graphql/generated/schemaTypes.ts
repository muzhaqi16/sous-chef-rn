...
export type Mutation = {
   ...
   * deleteShoppingListItems(purchased: true), a separate act the user confirms —
   * so a second call re-reads the same lines and finds them already stocked.
   *
   * Read the summary accordingly: succeeded = lines this call moved, skipped =
   * lines an earlier call had already moved (movedItems carries them too, with
   * alreadyInPantry: true), failed = lines that errored, each in failedItems
   * with its reason. A repeat call on an unchanged list reports succeeded: 0.
   *
