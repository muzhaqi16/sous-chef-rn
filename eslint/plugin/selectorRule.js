 ...
 * as a listener key, so matching is exactly `no-restricted-syntax`'s. A check
 * with `when` reports only the matches that predicate accepts.
 */
    ...
    create(context) {
      return Object.fromEntries(
        checks.map(c => [
          c.selector,
          node => {
            if (c.when && !c.when(node)) return;
            context.report({ node, messageId: c.messageId });
          },
        ]),
      );
    },
