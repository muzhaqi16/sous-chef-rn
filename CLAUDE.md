...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| A list that can grow                      | `FlashList`, with an explicit `renderScrollComponent`                                             | `no-restricted-imports` on `FlatList`/`SectionList` · `sous-chef/flashlist-declares-scroll-component`                                    |
...
| Text or an icon on a fill                 | that fill's `on*` token (`onScrim` over a ground the theme does not paint)                        | `sous-chef/on-fill-text-uses-its-token` · `onFillTextUsesItsToken.test.ts` (contrast) · `sous-chef/no-raw-color`                         |
...
- **Text or an icon on a fill reads that fill's `on*` token**, never a hardcoded white. `onScrim` is for a ground the theme doesn't paint, and there is no `colors.white` (`sous-chef/on-fill-text-uses-its-token`).
...
- **A FlashList whose rows carry RNGH gestures MUST render `renderScrollComponent={SwipeAwareScrollComponent}`.** RNGH v3 handlers survive a plain RN scroll takeover, and no `dragOffset` fixes it (`sous-chef/flashlist-declares-scroll-component`). Verified 2026-09-12 vs `react-native-gesture-handler@3.3.0` — `docs/verified-library-behaviour.md#rngh-v3-handlers-survive-a-native-scroll-takeover`.
